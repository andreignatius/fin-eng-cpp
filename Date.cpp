...
void Date::subtractMonths(int months) {
    ...
    date_tm->tm_mon -= months; // Subtract months
...
std::ostream &operator<<(std::ostream &os, const Date &date) {
    ...
    os << (1900 + ptm->tm_year) << "-" << std::setw(2) << std::setfill('0')
...
std::istream &operator>>(std::istream &is, Date &date) {
    int y, m, d;
    char delim1, delim2;
    if (is >> y >> delim1 >> m >> delim2 >> d && delim1 == '-' &&
        delim2 == '-') {
        date = Date(y, m, d);
    } else {
...
std::string Date::toString() const {
    ...
    oss << (1900 + ptm->tm_year) << "-" << std::setw(2) << std::setfill('0')
        << (ptm->tm_mon + 1) << "-" << std::setw(2) << std::setfill('0')
        << ptm->tm_mday;
    return oss.str();
}