...
double Pricer::Price(const Market &mkt, Trade *trade) {
    ...
    if (trade->getType() == "TreeProduct") {
        ...
        if (bondPtr) { // check if cast is sucessful
            ...
            const std::unordered_map<std::string, double> &mktBonds =
                mkt.getAllBondPrices();
            string bondName = bondPtr->getBondName();
            // cout << "start pricing with bond pricer for " << bondName <<
            // endl;

            auto it = mktBonds.find(bondName);
            if (it != mktBonds.end()) {
                price = it->second;
                // cout << "Bond Name: " << bondName << ", Price: " << price <<
                // endl;
            } else {
        ...
        if (swapPtr) { // check if cast is sucessful
            ...
            if (swapName != mktRates.getName()) {
                cout << "No market curve matched for " << swapName
                     << ", will price using default curve USD-SOFR!" << endl;
                cout << mktRates.getName() << endl;
            }
...
double BinomialTreePricer::PriceTree(const Market &mkt,
    ...
    */
    ...
    //     cout << "Asset: " << pair.first << ", Price: " << pair.second <<
    //     endl;
    // }
    ...
    if (it != stocks.end()) {
        ...
        // cout << "I've found the current market price: " << stockPrice <<
        // endl;
    } else {
        std::cerr << "Warning: Stock price not found for asset: "
                  << trade.getAssetName() << ". Defaulting to 0." << std::endl;
        return 0;
    }

    cout << "Model setup as follows:" << endl;
    cout << "S0=" << stockPrice << ", vol=" << vol << ", r=" << rate
         << ", T=" << T << ", steps=" << nTimeSteps << endl;
