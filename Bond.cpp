...
double Bond::PayoffCurve(RateCurve theCurve) const {
    ...
    // display the curve
    // std::cout << "CONSTRUCT BOND WITH CUSTOM CURVE" << std::endl;
    // theCurve.display();
    // 2. generate the bond schedule; CANNOT USE FUNCTION BECAUSE const
    // keyword
    /*
    ...
    for (int i = 0; i < numPeriods; ++i) {
        if (paymentSchedule[i] <= valueDate) {
            ...
            yearsSinceStart =
                ...
                Constants::NUM_DAYS_IN_YEAR; // Convert days to years
            rate = theCurve.getRate(paymentSchedule[i]);
